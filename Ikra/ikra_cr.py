...
def generate_verification_code():
    """Генерирует код проверки (4 символа: буквы, цифры и спецсимволы)."""
...
def generate_mark(with_separators=True, include_verification=True):
    ...
    mark = (
        f"{fnc1}01{gtin}{gs}21{serial_number}{gs}92{verification_code}"
        if with_separators else f"01{gtin}21{serial_number}92{verification_code}"
    )
    return mark
...
def create_segmented_marks(marks_with_separators):
    ...
    for mark in marks_with_separators:
        ...
        segmented_mark = {
            ...
            "**_VerificationCode_**": parts[2].replace("92", "") if len(parts) > 2 else ""
        }
...
output_file_path = os.path.join(os.path.dirname(__file__), "ikra_cr.txt")
